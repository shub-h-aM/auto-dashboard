...
function getBugReport() {
  ...
  var url = `/report/bug-report/1/${fromDateTimestamp}/${toDateTimestamp}`;
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Use a regular expression to extract the integer value after "Bug-report"
  var match = url.match(/\/bug-report\/(\d+)/);
  ...
  if (match) {
    ...
    console.log("Integer value after 'bug-report' not found in the URL");
    return; // Exit the function if the value is not found
  }

  const currentPageInput = document.getElementById("currentBugPage");
  ...
  if (nextPageLink) {
    nextPageLink.addEventListener("click", function (event) {
      ...
      const currentBugPage = parseInt(
        document.getElementById("currentBugPage").value
      );

      // Calculate the next page
      const nextPage = currentBugPage + 1;
      const fromDateTimestamp = Date.parse(fromDate);
      const toDateTimestamp = Date.parse(toDate);

      // Construct the URL with the selected dates and page number
      const url = `/report/bug-report/${nextPage}/${fromDateTimestamp}/${toDateTimestamp}`;
  ...
  if (previousPageLink) {
    previousPageLink.addEventListener("click", function (event) {
      ...
      const previousPage = currentBugPage - 1;
      const fromDateTimestamp = Date.parse(fromDate);
      const toDateTimestamp = Date.parse(toDate);
      ...
      // Construct the URL with the selected dates and page number
      const url = `/report/bug-report/${revisedPreviousPage}/${fromDateTimestamp}/${toDateTimestamp}`;
  ...
  numberNextPageLinks.forEach(function (numberNextPageLink) {
    numberNextPageLink.addEventListener("click", function (event) {
      ...
      // Construct the URL with the selected dates and page number
      const url = `/report/bug-report/${pageNumber}/${fromDateTimestamp}/${toDateTimestamp}`;
